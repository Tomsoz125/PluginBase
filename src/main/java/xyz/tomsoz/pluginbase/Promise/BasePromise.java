...
import xyz.tomsoz.pluginbase.Exceptions.PromiseChainException;
...
final class BasePromise<V> implements Promise<V> {
    ...
    private void executeDelayedSync(@NotNull final Runnable runnable, final long delayTicks) {
        if (delayTicks <= 0) {
            ...
            Bukkit.getScheduler()
                    .runTaskLater(PluginManager.getPlugin(), SchedulerTaskException.wrap(runnable), delayTicks);
        }
    }
    ...
    private void executeDelayedAsync(@NotNull final Runnable runnable, final long delayTicks) {
        if (delayTicks <= 0) {
            ...
            Bukkit.getScheduler().runTaskLaterAsynchronously(PluginManager.getPlugin(),
    ...
    private void executeDelayedSync(@NotNull final Runnable runnable, final long delay,
        ...
        if (delay <= 0) {
            ...
            Bukkit.getScheduler()
                    .runTaskLater(PluginManager.getPlugin(), SchedulerTaskException.wrap(runnable),
